		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) { /*
			...
			if (lpparam.packageName.equals(PKGNAME_TELECOM)) {
				...
				try {
					...
					// mCallState = XposedHelpers.findClass(CLASSNAME_CALLSTATE,
					// lpparam.classLoader);
				...
				try {
					...
					// mCallState = XposedHelpers.findClass(CLASSNAME_CALLSTATE,
					// lpparam.classLoader);
					onCallAdded = XposedHelpers.findMethodExact(mPhoneStateBroadcaster, "onCallAdded", mCall) == null ? false
							: true;
				} catch (Exception e) {
				}
								...
								try {
									...
									if (debug)
										XposedBridge.log(String.format(
												"onCallStateChanged: oldState:%s newState:%s context:%b",
					...
					protected void afterHookedMethod(MethodHookParam param) throws Throwable {
						try {
							...
							if (pref.getBoolean("pref_vibrate_call_waiting", true)) {
								...
								if (debug)
									XposedBridge.log(String.format("onCallAdded: state:%s", CallState.toString(state)));
