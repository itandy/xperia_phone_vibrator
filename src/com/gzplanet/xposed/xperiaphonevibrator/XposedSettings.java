...
public class XposedSettings extends Activity {
	...
	public static class PrefMainFragment extends PreferenceFragment {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			mVibrateEveryMinuteSecond = Integer.valueOf(getPreferenceManager().getSharedPreferences().getString("pref_vibrate_every_minute_second", "45"));

			mPrefEveryMinute = (CheckBoxPreference) findPreference("pref_vibrate_every_minute");
			mPrefEveryMinute.setSummary(String.format(getActivity().getResources().getString(R.string.summ_vibrate_every_minute), mVibrateEveryMinuteSecond));

			mPrefEveryMinuteSecond = (EditTextPreference) findPreference("pref_vibrate_every_minute_second");
			mPrefEveryMinuteSecond.getEditText().setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_SIGNED);
				...
				public boolean onPreferenceChange(Preference preference, Object newValue) {
					try {
						if (Integer.valueOf(newValue.toString()) <= 0 || Integer.valueOf(newValue.toString()) >= 60) {
							Toast.makeText(getActivity(), getActivity().getResources().getString(R.string.text_invalid_value), Toast.LENGTH_LONG).show();
							return false;
						}
					} catch (Exception e) {
						Toast.makeText(getActivity(), getActivity().getResources().getString(R.string.text_invalid_value), Toast.LENGTH_LONG).show();
					...
					mPrefEveryMinute.setSummary(String.format(getActivity().getResources().getString(R.string.summ_vibrate_every_minute),
			...
			mVibrateFixedTimeSecond = Integer.valueOf(getPreferenceManager().getSharedPreferences().getString("pref_vibrate_fixed_time_second", "0"));
			...
			mPrefFixedTimeSecond.getEditText().setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_SIGNED);
			...
			mPrefFixedTimeSecond.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
				...
				public boolean onPreferenceChange(Preference preference, Object newValue) {
					...
					mPrefFixedTime.setSummary(String.format(getActivity().getResources().getString(R.string.summ_vibrate_fixed_time),
			...
			mPrefVibrateIntensity.setOnPreferenceClickListener(new OnPreferenceClickListener() {
				...
				public boolean onPreferenceClick(Preference preference) {
					...
					getActivity().getFragmentManager().beginTransaction().addToBackStack(null).replace(android.R.id.content, fragVibrationIntensity).commit();
					return true;
				}
			});
	...
	public static class PrefVibrationIntensityFragment extends PreferenceFragment {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			findPreference("pref_hangup_vibrate_intensity").setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
			...
			findPreference("pref_fixed_time_vibrate_intensity").setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
